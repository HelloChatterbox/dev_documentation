...
Putting the example above together in an otherwise unmodified User level `chatterbox.conf`, would be:
...
For the `Yo Mike` example we started with, an example `~/.chatterbox/chatterbox.conf` file might look like:
